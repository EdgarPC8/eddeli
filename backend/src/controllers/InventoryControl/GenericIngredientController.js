...
export const linkPresentation = async (req, res) => {
  try {
    ...
    if (!target || (target.type !== "final" && !(target.isGenericIngredient && target.type === "raw"))) {
      notifyFail("presentation.link_failed", "Destino de apertura inválido", { req, httpStatus: 400 });
      return res.status(400).json({
        message: "El destino debe ser un insumo genérico o un producto final.",
      });
    }
    ...
    if (!Number.isInteger(units) || units < 1) {
      return res.status(400).json({
        message: "Unidades por paca debe ser un número entero mayor o igual a 1.",
      });
    }
